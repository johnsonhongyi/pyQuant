...
def get_roll_mean_all(single=True,tdx=False):
    ...
    code_uniquelist=df.index.get_level_values('code').unique()
    code_select = code_uniquelist[random.randint(0,len(code_uniquelist)-1)]
    print round(time.time()-time_s,2),'s',df.index.get_level_values('code').unique().shape,code_select,df.loc[code_select].T
    ...
    if single:
        ...
        if tdx:
            ...
            # else:
            codew = df[(df.date >= cct.last_tddate(days=5)) & (df.date <= cct.get_today())].index.tolist()
        ...
        print df.loc[code_select],df.shape
        MultiIndex = False
    else:
        MultiIndex = True
...
if __name__ == '__main__':
    get_roll_mean_all(single=True,tdx=True)