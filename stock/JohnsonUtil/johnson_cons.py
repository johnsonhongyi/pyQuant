...
TODAY_TICK_COLUMNS = ['time', 'price', 'pchange',
...
ALL_STOCK_BASICS_FILE = '%s%s/static/all.csv' % (
    P_TYPE['http'], DOMAINS['oss'])
...
SINA_Market_Clean_UP_ColumnsTrade = ['name', 'trade', 'dff', 'volume', 'percent', 'ratio', 'high', 'open', 'low', 'couts',
                                     'prev_p']
SINA_Market_Clean_Columns = ['name', 'buy', 'dff', 'volume',
                             'percent', 'ratio', 'high', 'open', 'low', 'couts']
# SINA_Market_Clean_UP_Columns=['name','buy','dff','percent','trade','high','ratio','open','low','couts']

THE_FIELDS = ['code', 'symbol', 'name', 'changepercent', 'trade',
              'open', 'high', 'low', 'settlement', 'volume', 'turnoverratio']
# Market_Center_COLUMNS = ['code','name','trade','changepercent','buy','sell','settlement','open','high','low','volume','ticktime','turnoverratio']
DAY_REAL_DD_COLUMNS = ['code', 'symbol', 'name',
                       'ticktime', 'price', 'volume', 'prev_price', 'kind']
Status_DD = {u"中性盘": "normal", u"买盘": "up", u"卖盘": "down"}
Status_KIND = ['U', 'D', 'E']

SINA_JSON_API_URL = "http://hq.sinajs.cn/list=%s"
SINA_Market_KEY_TO_DFCFW = {
    'sh': 'zs000001', 'sz': 'zs399001', 'cyb': 'zs399006', 'zxb': 'zs399005'}
SINA_Market_KEY_TO_DFCFW_New = {
    'sh': '0000011', 'sz': '3990012', 'cyb': '3990062', 'zxb': '3990052'}
...
INDEX_COLS = ['code', 'name', 'change', 'open',
              'preclose', 'close', 'high', 'low', 'volume', 'amount']
HIST_FQ_COLS = ['date', 'open', 'high', 'close',
                'low', 'volume', 'amount', 'factor']
...
#        idx_key = [ idx_list.index(x) for x in perd_l if x in idx_list]
        ...
        if len(idx_key) == 0:
            idx_key = [column.index(x) for x in column if x.find(dest) > -1]
        ...
        if len(t_list) == len(duration_format):
            duration_format = t_list
        if not dest in duration_format:
...
def get_Dynamic_Duration_perd_org(market_sort_value, idx_perd, columns=None):
    ...
    idx_perd = int(idx_perd)
    if idx_perd > 1:
        if idx_perd > 3 and columns is not None and len(columns) > 0:
            idx_k = idx_perd
            if not idx_value2 % idx_k in columns:
