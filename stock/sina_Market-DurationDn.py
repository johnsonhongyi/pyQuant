...
#            top_now = tdd.getSinaAlldf(market=u'次新股',filename='cxg', vol=ct.json_countVol, vtype=ct.json_countType)
            ...
            if len(top_now) > 0 or cct.get_work_time():
                ...
                if len(top_dif) == 0:
                    ...
                    # elif percent_status == 'y' and cct.get_now_time_int() >
                    # 935 and ptype == 'high' :
                    elif ptype == 'low':
                        # top_dif = top_dif[top_dif.percent >= 0]
                        top_temp = top_dif[:ct.PowerCount].copy()
                    ...
                    if 'op' in top_temp.columns:
                        ...
                        # if cct.get_now_time_int() > 945 and int(duration_date
                        # * 1.5)  > ct.duration_date_sort:
            ...
            if len(st) == 0:
                ...
                codew = stf.WriteCountFilter(
                    top_temp, duration=duration_date, writecount=args.dl)
                if args.code == 'a':
