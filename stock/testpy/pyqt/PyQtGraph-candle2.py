...
# https://blog.csdn.net/otter1010/article/details/103396064?depth_1-utm_source=distribute.pc_relevant.none-task&utm_source=distribute.pc_relevant.none-task
# 字符串转换
# ---------------------------------------------------------------------------------------
try:
...
class GetData():
    ...
    def getData_Tushare(self, code, ktype='5', start=str(dt.date.today() - dt.timedelta(days=1000)),
        ...
        try:
            ...
            self.k_data['ma5'] = talib.EMA(self.k_data['close'].values, timeperiod=5)
            self.k_data['ma10'] = talib.EMA(self.k_data['close'].values, timeperiod=10)
            self.k_data['ma20'] = talib.EMA(self.k_data['close'].values, timeperiod=20)
            self.k_data['Boll_Up'], self.k_data['Boll_Mid'], self.k_data['Boll_Down'] = \
                talib.BBANDS(self.k_data['close'].values, timeperiod=20, nbdevup=2, nbdevdn=2, matype=0)
