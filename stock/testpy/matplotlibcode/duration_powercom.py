...
def graphData_candles(stock, MA1, MA2, start='2016-01-01'):
    '''
    ...
    try:
        ...
        # candlestick_ohlc(ax1, newAr[-SP:], width=.6, colorup='#53c156',
        # colordown='#ff1717')
...
def graphData_candles2(stock, MA1, MA2, start='2016-01-01'):
    ...
    try:
        ...
        candlestick2_ohlc(ax1, openp[-SP:], highp[-SP:], lowp[-SP:], closep[-SP:],
                          width=0.8, colorup='#ff1717', colordown='#53c156', alpha=1.0)
        # volume_overlay(ax1, openp[-SP:], closep[-SP:], volume[-SP:], colorup='k', colordown='r', width=4, alpha=1.0)
        # fig = figure()
        # ax = fig.add_subplot(111)
        # candlestick2(ax,bars['open'],bars['close'],bars['high'],bars['low'],width=.5,colorup='g',colordown='r',alpha=1)
        ...
        ax0.axhline(70, color=negCol)
        ax0.axhline(30, color=posCol)
        ax0.fill_between(idx_a, rsi[-SP:], 70, where=(rsi[-SP:] >= 70),
                         facecolor=negCol, edgecolor=negCol, alpha=0.5)
        ax0.fill_between(idx_a, rsi[-SP:], 30, where=(rsi[-SP:] <= 30),
        ...
        ax1v = ax1.twinx()
        ax1v.fill_between(idx_a, volumeMin,
                          volume[-SP:], facecolor='#00ffe8', alpha=.4)
        ax1v.axes.yaxis.set_ticklabels([])
        ax1v.grid(False)
        # Edit this to 3, so it's a bit larger
        ...
        ax2.fill_between(idx_a, macd[-SP:] - ema9[-SP:], 0,
                         alpha=0.5, facecolor=fillcolor, edgecolor=fillcolor)
...
graphData_candles2('sh', 10, 20)

graphData_candles('sh', 10, 20)
time.sleep(555)
