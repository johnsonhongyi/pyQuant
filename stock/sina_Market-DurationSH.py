...
if __name__ == "__main__":
    ...
    market_sort_value, market_sort_value_key = ct.get_market_sort_value_key(ct.sort_value_key_percd)
    while 1:
        try:
...
#            top_now = tdd.getSinaAlldf(market='all', vol=ct.json_countVol, vtype=ct.json_countType)
            ...
            if len(top_now) > 10 or cct.get_work_time():
                ...
                if len(top_dif) == 0:
                    ...
                    print "Rt:%0.1f dT:%s N:%s T:%s %0.1f%%" % (float(time.time() - time_Rt), cct.get_time_to_date(time_s), cct.get_now_time(), len(top_temp), round(len(top_temp) / float(ct.PowerCount) * 100, 1))
