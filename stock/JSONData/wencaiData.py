...
#        url="http://www.iwencai.com/stockpick/search?typed=0&preParams=&ts=1&f=1&qs=result_original&selfsectsn=&querytype=&searchfilter=&tid=stockpick&w=%E6%9C%89%E8%89%B2+%E7%85%A4%E7%82%AD"
        ...
        if len(count) == 1:
            ...
            if perpage > 1000:
                html = cct.get_url_data(cacheurl,retry_count=1,headers=headers,timeout=30)
            else:
                html = cct.get_url_data(cacheurl,retry_count=1,headers=headers,timeout=10)
            # js2py_test(cacheurl)
...
#                    log.info( key_t)
                ...
                # df = pd.DataFrame(dlist, columns=['category','code','name','trade','percent','tilte1','tilte2'])
                        ...
                        if len(code_t) > 0 and code_t[0].endswith(('SZ', 'SH')):
                            ...
                            singlelist.append({'code': code_t[0].split('.')[0], 'name': code_t[1], 'trade': code_t[2], 'percent': code_t[3]})
                if len(singlecode)>0:
                    ...
                    wencaisingleurl = wencaisingleroot%(codestring)+end_root
                    html_json = cct.get_url_data(wencaisingleurl,retry_count=1,headers=headers,timeout=10)
                    dr = re.compile(r'<[^>]+>',re.S)
